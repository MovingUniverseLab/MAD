...
def get_moa_lightcurves(year):
    ...
    for nn, alert_dir in enumerate(alert_dirs):
        ...
        # and telescope (MOA)
        # need to make sure this always works.
        df['alert_name'] = 'MB' + year[2:] + str(nn + 1).zfill(3)
        df['telescope'] = 'MOA'
        ...
        # df[cols].to_sql(con=engine, schema=None,
        #                name="photometry", if_exists="append", index=False)
        print("GET_MOA_LIGHTCURVES  Saving Lightcurves into temporary CSV Files")
        df.to_csv('data_photometry/lc_moa_' +
                  str(df['alert_name'].iloc[0])+'.csv')

    t1 = time.time()

    print('Read', len(alert_dirs),
          ' light curves from MOA in {0:.2f} seconds'.format(t1-t0))
...
def get_ogle_lightcurves(year):
    ...
    for i_pref, pref in enumerate(prefs):
        for nn in np.arange(start=1, stop=nobjs[i_pref]+1, step=1):
            ...
            # and telescope (OGLE)
            df['alert_name'] = 'O' + pref[0].upper() + year[2:] + \
                str(nn).zfill(4)
            df['telescope'] = 'OGLE'
    ...
    print('Read', sum(nobjs),
          'light curves from OGLE in {0:.2f} seconds'.format(t1-t0))
...
def get_kmtnet_lightcurves(year):
    ...
    print('Read', nobj,
          'light curves from KMTNet in {0:.2f} seconds'.format(t1-t0))
...
def get_moa_params(alert_dir, year, nn):
    ...
    tmax_str = soup.find('div', id="lastphot").text.split(
        '<td>=<td align=right>')[1]
    tmax = moa_str_to_float(tmax_str.split()[1])
    tmax_e = moa_str_to_float(tmax_str.split('<td>')[2].split()[0])

    tE_str = soup.find('div', id="lastphot").text.split(
        '<td>=<td align=right>')[2]
    tE = moa_str_to_float(tE_str.split()[0])
    tE_e = moa_str_to_float(tE_str.split('<td>')[2].split()[0])

    u0_str = soup.find('div', id="lastphot").text.split(
        '<td>=<td align=right>')[3]
    u0 = moa_str_to_float(u0_str.split()[0])
    u0_e = moa_str_to_float(u0_str.split('<td>')[2].split()[0].split('<')[0])

    Ibase_str = soup.find('div', id="lastphot").text.split(
        '<td>=<td align=right>')[4]
    Ibase = moa_str_to_float(Ibase_str.split()[0])
    Ibase_e = moa_str_to_float(Ibase_str.split(
        '<td>')[2].split()[0].split('<')[0])

    assessment = soup.find('div', id="metadata").find_all(
        'td', align='right')[4].text
...
def get_moa_alerts(year):
    ...
    # SKY next 7 lines replace previous 3.
    session = requests.Session()
    try:
        response = session.get(url)
        response.raise_for_status()
        html = response.content  # using .content to get bytes, .text for unicode
    finally:
        response.close()
    ...
    print("Entering multi-threading")
    try:
    ...
    moa2ogle_url = 'http://www.massey.ac.nz/~iabond/moa/alert' + \
        year+'/fetchtxt.php?path=moa/alert'+year+'/moa2ogle.txt'
    moa2ogle_df = pd.read_csv(BytesIO(requests.get(moa2ogle_url).content), usecols=(0, 2),
                              sep='\s+', skiprows=6, skipfooter=1, header=None, engine='python',
                              names=['name1', 'name2'])
    moa2ogle_pairs = np.sort(np.transpose(
        [moa2ogle_df['name1'], moa2ogle_df['name2']]))
    moa2ogle_cols = np.transpose(moa2ogle_pairs)
    moa_evs = [moa_ev[0]+moa_ev[9]+year[2:]+moa_ev[-3:]
               for moa_ev in moa2ogle_cols[0]]
    ogle_evs = [ogle_ev[0]+ogle_ev[10]+year[2:]+ogle_ev[-4:]
                for ogle_ev in moa2ogle_cols[1]]
    rel_evs = []
    ...
    # df.to_sql(con=engine, schema=None, name="alerts",
    #           if_exists="append", index=False)

    _t1 = time.time()
    print('Read', len(df['alert_name']),
          'MOA alerts in {0:.2f} seconds'.format(_t1-_t0))
    print("Saving temporary file:")
    df.to_csv('data_alerts/moa_alerts.csv', index=False)
...
def get_ogle_alerts(year):
    ...
    pool = mp.Pool(processes=num_workers)
    try:
        parallel_results_blg = pool.starmap(
            get_ogle_params, zip(repeat(year), range(nblg), repeat('BLG')))
        parallel_results_dg = pool.starmap(
            get_ogle_params, zip(repeat(year), range(ndg), repeat('DG')))
        parallel_results_gd = pool.starmap(
            get_ogle_params, zip(repeat(year), range(ngd), repeat('GD')))
        parallel_results = parallel_results_blg + \
            parallel_results_dg + parallel_results_gd
    finally:
        pool.close()
        pool.join()
    ...
    df.to_csv('data_alerts/ogle_alerts.csv', index=False)
...
def get_kmtnet_alerts(year):
    ...
    c = SkyCoord(ra=RA_list, dec=Dec_list, unit=(
    ...
    if year in years_oneclass:
        ...
        classEF_list = [item.get_text().replace(u'\xa0', u'')
                        for item in classEF]
        classAL_list = [item.get_text().replace(u'\xa0', u'')
                        for item in classAL]
    ...
    alert_url_list = [
        kmt_alert_url + item.find_all('a', href=True)[0]['href'] for item in alert_url]
    ...
    print('Read', len(df['alert_name']),
          'KMTNet alerts in {0:.2f} seconds'.format(_t1-_t0))
    print("Writing to temp csv file")
    print(df.to_csv('data_alerts/kmtnet_alerts.csv', index=False))
